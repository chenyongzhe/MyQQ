...
import com.chen.a12763.myqq.DataBase.FriendDao;
import com.chen.a12763.myqq.DataBase.SaveMsgDao;
import com.chen.a12763.myqq.DataBase.UserDao;
import com.chen.a12763.myqq.server.ServerListen;
...
public class ClientActivity {
	...
	public void update_info(TranObject tran) {
		User user = (User) tran.getObject();
		System.out.println("user 更新用户名"+user.getUserName());
		System.out.println("user 更新用户签名"+user.getUserBriefIntro());
		//int id = UserDao.insertInfo(user);
		if(UserDao.updateUser_info(user)){

			tran.setResult(Result.UPDATE_USER_INFO_SUCCESS);
		}else {
			tran.setResult(Result.UPDATE_USER_INFO_FAIL);
		}
		//user.setId(id);
		/*
		if (id == -1)
