...
public class UserDao {
	...
	public static boolean login(User user) {
		...
		try {
			...
			if (rs.first()) {
				...
				user.setAge(getAge(rs.getDate("birthday").toString()));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} catch (Exception e) {
	...
	public static boolean updateUser_info(User user) {
		String sql0 = "use myqq";
		String sql1 = "update user SET name=?,gender=?,location=? where account=?";
		Connection con = DBPool.getConnection();
		try {
			con.setAutoCommit(false);
		} catch (SQLException e1) {
